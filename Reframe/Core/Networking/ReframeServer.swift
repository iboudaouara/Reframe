...
final class ReframeServer {
    ...
    static let jsonEncoder: JSONEncoder = {
            ...
            encoder.dateEncodingStrategy = .iso8601 // 👈 Important pour le serveur
            return encoder
        }()

        static let jsonDecoder: JSONDecoder = {
            let decoder = JSONDecoder()
            decoder.dateDecodingStrategy = .iso8601 // 👈 Important pour lire la réponse
            return decoder
        }()
    ...
    func request<T: Decodable>(
        ...
        if let body = body {
            request.httpBody = try ReframeServer.jsonEncoder.encode(body)
        }
        ...
        return try ReframeServer.jsonDecoder.decode(T.self, from: data)
    }
...
struct TacticalUploadBody: Encodable {
    let situation: String
...
extension ReframeServer {
    ...
    func saveTacticalSession(_ session: TacticalAnalysis, token: String) async throws -> Int {
        // On prépare l'objet Encodable plutôt que le dictionnaire [String: Any]
        let body = TacticalUploadBody(
            situation: session.situation,
            maneuver_name: session.maneuverName,
            maneuver_description: session.maneuverDescription,
            power_score: session.powerScore,
            emotional_impact: session.emotionalImpact,
            moves: session.recommendedMoves.map {
                TacticalUploadBody.MoveBody(text: $0.text, success_rate: $0.successRate)
            }
        )
        
        // On précise les headers pour inclure le Token s'il n'est pas déjà géré
        let headers = ["Authorization": "Bearer \(token)"]
